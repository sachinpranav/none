...
function ajaxQtyCart()
{
    ...
    $newQty = filter_var(filter_input(INPUT_POST, 'quantity'), FILTER_SANITIZE_NUMBER_INT);

    // Get the array of values owned by the product we're updating
    $item = WC()->cart->get_cart_item($itemKey);

    // Get the quantity of the item in the cart
    $qty = apply_filters('woocommerce_stock_amount_cart_item', apply_filters( 'woocommerce_stock_amount', preg_replace( "/[^0-9\.]/", '', $newQty) ), $itemKey);
...
function registerCartScripts()
{
    wp_register_script('breakdance-woo-cart', BREAKDANCE_WOO_URL . 'js/cart.js', [], false, true);
}
add_action( 'init', '\Breakdance\WooCommerce\registerCartScripts');

function enqueueCartScripts()
{
    wp_enqueue_script('breakdance-woo-cart');
}
add_action('woocommerce_before_cart', '\Breakdance\WooCommerce\enqueueCartScripts');
