...
class TextLink extends \Breakdance\Elements\Element
{
    ...
    {
        return [['name' => 'href', 'template' => '{{ macros.getUntrimmedLinkUrl(content.content.link)|trim }}'], ['name' => 'target', 'template' => '{{ content.content.link.openInNewTab ? \'_blank\' : \'_self\' }}'], ['name' => 'data-lightbox-id', 'template' => '{{ content.content.link.type == \'lightbox\'? \'%%ID%%\' }}'], ['name' => 'data-content-editable-property-path', 'template' => '{% if not content.content.tag %}content.content.text{% endif %}'], ['name' => 'data-type', 'template' => '{{ content.content.link.type }}'], ['name' => 'data-action', 'template' => '{{ content.content.link.action|json_encode }}']];
    }
