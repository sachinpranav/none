...
)
{
    ...
    echo '<div class="bde-posts-pagination bde-posts-navigation-numbers">';
...
function getPostsPaginationFromProperties($propertiesData, $maxNumPages, $layout, $ssrFilePath){
    ...
    if (isAnyArchive() && $propertiesData['content']['query']['query']) {
        return;
    }
    ...
    if (isAnyArchive() &&
        ...
        if (isRequestFromBuilderSsr()) {
            echo "<div class='bde-no-custom-query-error'>Dynamic pagination isn't allowed in Archives.</div> ";
        }
        ...
        switch ($selectedPagination) {
            case 'numbers':
                getNumberedAndOrPrevNextPagination(
                    $maxNumberOfPages,
                    $pageNumber,
                    $pagination['show_all_page_numbers'],
                    false,
                );
                break;
            case 'prevnext':
