...
function getFormOptions($ajaxHandler, $settings)
{
    ...
    return [
        ...
        'redirect' => $settings['form']['redirect'],
        'redirectUrl' => $settings['form']['redirect_url'],
        'customJavaScript' => $settings['actions']['custom_javascript'],
        'recaptcha' => [
            'key' => $recaptchaSiteKey ?? '',
            'enabled' => $settings['advanced']['recaptcha']['enabled'],
        ],
        'honeypot_enabled' => $settings['advanced']['honeypot_enabled'],
...
function getFieldAttributes($field)
{
     ...
     * @psalm-suppress MixedArrayAccess
     */
    $id = $field['advanced']['id'] ?: sanitize_title($label);
