...
const FIELDS_EXCLUDED_FROM_EXPORT = ['file'];
...
function getFieldsForHeaderRow($formSubmissions)
{
    ...
    foreach ($settings['form']['fields'] as $field) {
        if (in_array($field['type'],FIELDS_EXCLUDED_FROM_EXPORT)) {
            continue;
        }
...
function handleExport()
{
    ...
    if (array_key_exists('form_id', $_GET) && $_GET['form_id'] !== '') {
        exportSingleFormAsCsv((int) $_GET['form_id']);
        exit();
    }
 ...
 * @return void
 */
function exportSingleFormAsCsv($formId)
{
    ...
    outputSubmissionsAsCsv($formId, $csvFileHandler);
...
function exportMultipleFormsAsZip()
{
    ...
    $temporaryFile = wp_tempnam("tmp", "breakdance-submission-export-zip");

    $siteDomain = parse_url(site_url(), PHP_URL_HOST);
    $currentDate = new \DateTime();
    $csvOutputFilenameFormat =  '%s-submissions-export-%s-%s.csv';
    ...
    foreach ($formsWithSubmissions as $form) {
        $filePath = wp_tempnam("tmp", "breakdance-submission-export-csv");
        $csvFile = fopen($filePath, 'wb');
        $submissionCount = outputSubmissionsAsCsv($form['formId'], $csvFile);
        $csvOutputFilename = sprintf($csvOutputFilenameFormat, $siteDomain, $form['formId'], (new \DateTime())->format('Y-m-d-G-i'));
        fclose($csvFile);
 ...
 * @return int
 */
function outputSubmissionsAsCsv($formId, $fileHandler)
{
    global $post;
    ...
    if ($formId) {
        $queryArguments['meta_key'] = '_breakdance_form_id';
        $queryArguments['meta_value'] = $formId;
    }
    ...
    while (true) {
        ...
        while ($formSubmissions->have_posts()) {
            ...
            $submissionValues = getFieldsForRow($headers, $post->ID, $settings['form']['fields'], $meta);
 ...
 * @return array
 */
function getFieldsForRow($headers, $postId, $fields, $meta)
{
    $submissionValues = [];
    foreach ($headers as $fieldId => $fieldLabel) {
        $fieldValue = '';
        if (array_key_exists($fieldId, $meta['fields']) && is_string($meta['fields'][$fieldId])) {
    ...
    $submissionValues['formId'] = $postId;
