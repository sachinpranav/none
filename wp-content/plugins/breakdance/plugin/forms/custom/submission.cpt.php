...
function renderColumns($column, $postId)
{
    ...
    switch ($column) {
        case 'form':
            if ($meta['formId'] && $meta['builderUrl']) {
                echo sprintf('<a href="%s" target="_blank">%s (%s)</a>', $meta['builderUrl'], $meta['formName'], $meta['formId']);
            } else {
    ...
    <table class="form-table">
       <tbody>
        ...
        foreach ($meta['form'] as $field) {
            $id = sprintf("fields[%s]", $field['advanced']['id']);
            ...
            $fieldValue = '';
            if (isset($meta['fields'][$field['advanced']['id']])) {
                /** @var string|string[] $fieldValue */
                $fieldValue = $meta['fields'][$field['advanced']['id']];
            }
...
function renderSidebar()
{
    ...
    <div id="minor-publishing">
        <ul>
            ...
            <?php if ($meta['editUrl']) { ?>
                <li>Post: <strong><?php echo sprintf('<a href="%s" target="_blank">%s</a>', $meta['editUrl'], $meta['postTitle']); ?></strong></li>
            <?php } ?>
            <?php if ($meta['user']) { ?>
...
function addMetaBoxes()
{
    ...
    if ($post !== null) {
        ...
        if (!empty($attachments)) {
            add_meta_box(
                    'breakdance_form_res_files',
                    'Files',
                    'Breakdance\Forms\Submission\renderFilesMetaBox',
                    'breakdance_form_res',
                    'advanced',
                    'default',
                    compact('attachments')
...
function parseFormFilter($query)
{
    ...
    $postType = (string) ($_GET['post_type'] ?? null);

    if ($postType !== 'breakdance_form_res' || !is_admin() || empty($formId)) {
        return;
    }

    $query->query_vars['meta_key']     = '_breakdance_form_id';
...
function addFormFilter($postType)
{
    ...
    $selected = (int) ($_GET['form_id'] ?? null);
    ...
    foreach ($results as $result) {
        if (isset($forms[$result['formId']]) && !empty($forms[$result['formId']])) {
            // if form ID is present with a form name value there's no need to include it again
            continue;
        }

        $settings = getFormSettings($result['postId'], $result['formId']);
        if (!$settings) {
            // Form has likely been deleted, we can attempt
            // to get the form name from the first submission
            $name = getFormNameFromLatestSubmissionSettings($result['formId']);
        } else {
            $name = $settings['form']['form_name'];
        }

        $forms[$result['formId']] = $name;
    }

    $options = array_map(static function ($formName, $formId) use ($selected) {
        if ($formId === $selected) {
            return sprintf('<option value="%1$s" selected>%2$s (%1$s)</option>', $formId, $formName);
        } else {
            return sprintf('<option value="%1$s">%2$s (%1$s)</option>', $formId, $formName);
        }
 ...
 * @return string
 */
function getFormNameFromLatestSubmissionSettings($formId)
{
    $latestSubmissions = wp_get_recent_posts([
        'post_type' => 'breakdance_form_res',
        'post_status' => 'any',
        'posts_per_page' => 1,
        'meta_key' => '_breakdance_form_id',
        'meta_value' => $formId
    ], 'OBJECT');
...
function getUniqueFormsFromSubmissions() {
    ...
    $statusFilter = (string) filter_input(INPUT_GET, 'post_status', FILTER_SANITIZE_STRING);
    if ($statusFilter) {
        /**
         * @psalm-suppress MixedPropertyFetch
         */
 ...
 * @return void
 */
function renderFilesMetaBox($post, $metaBox)
{
            ...
            foreach ($attachments as $attachment) {
                ...
                <tr>
                    <td>
                        ...
                        <span class="breakdance-attachment-item__attachment"><a target="_blank" href="<?php echo (string) wp_get_attachment_url($attachment->ID); ?>"><?php echo (string) $attachment->post_title; ?></a></span>
                    </td>
                </tr>
            <?php } ?>
            </tbody>
...
function addCsvExportButton($where)
{
    ...
    if ($post_type_object->name === 'breakdance_form_res') {
        ...
        $exportUrlArgs = http_build_query(array_merge(
                $_GET,
                ['breakdance_action' => 'export_submissions_to_csv',
                '_wpnonce' => $wp_nonce]
        ));

        $exportUrl = add_query_arg($exportUrlArgs, '', home_url($wp->request));
        if (\Breakdance\Forms\Submission\hasActiveFilters()) {
            echo '<div class="alignleft actions"><a class="button" href="' . $exportUrl . '">Export filtered to CSV</a></div>';
        } else {
            echo '<div class="alignleft actions"><a class="button" href="' . $exportUrl . '">Export all to CSV</a></div>';
        }
    }
}
