...
add_action('breakdance_admin_menu', function () {
   ...
   */
  add_submenu_page(null, 'Setup Wizard', 'Setup Wizard', 'manage_options', 'breakdance_setup_wizard', "Breakdance\SetupWizard\display_setup_wizard");
});
    ...
    if (is_post_request() && check_admin_referer($nonce_action)) {
        ...
        if ($key !== null && $key !== false) {
            ...
            if (strlen($trimmed_key)) {
                ...
                if (is_array($key_remote_info) && ($key_remote_info['success'] ?? false) === false) {
                    $license_info = get_human_readable_license_info_by_key_remote_info($key_remote_info);

                    $key_error = $license_info['activation_status'];
                }
