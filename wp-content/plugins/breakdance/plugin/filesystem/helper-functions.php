 ...
 * @throws Exception
 */
function validate_bucket_exists(string $bucket)
{
    if (!array_key_exists($bucket, BREAKDANCE_FS_BUCKETS)) {
...
function get_bucket_abs_path(string $bucket): string
{
    ...
    $bucket_uploads_subdir_path = wp_normalize_path(BREAKDANCE_FS_BUCKETS[$bucket]);

    return path_join(
        $breakdance_uploads_dir,
        $bucket_uploads_subdir_path
    );
}
...
function get_bucket_url(string $bucket): string
{
    ...
    /** @var string $bucket_uploads_subpath */
    $bucket_uploads_subpath = BREAKDANCE_FS_BUCKETS[$bucket];

    return join_url_path_parts($breakdance_uploads_url, $bucket_uploads_subpath);
}
