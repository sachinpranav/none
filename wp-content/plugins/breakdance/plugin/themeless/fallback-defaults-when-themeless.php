 ...
 * @throws \Exception
 */
function renderElementHtml($type, $childHtml = '', $classString = '')
{
    $element = \Breakdance\Render\getElementFromNodeType($type);
    $classes = [\Breakdance\Elements\getBaseClassNameForBuilderElement($element), $classString];

    return \Breakdance\Render\getHtml(
        false,
        $element,
        [],
        $childHtml,
 ...
 * @throws \Exception
 */
function wrapWithSection($html, $classString = "")
{
    $elementAsString = 'EssentialElements\\Section';
    $element = \Breakdance\Render\getElementFromNodeType($elementAsString);
    $dependencies = ['inlineStyles' => [$element::defaultCss()]];

    ScriptAndStyleHolder::getInstance()->append($dependencies);

    return renderElementHtml($elementAsString, $html, $classString);
}
