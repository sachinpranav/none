...
use function Breakdance\Licensing\activate_stored_license_key_at_edd;
use function Breakdance\Licensing\check_license_key_at_edd_and_upgrade_trial_if_successful;
use function Breakdance\Licensing\deactivate_stored_license_key_at_edd;
use function Breakdance\Licensing\get_human_readable_license_info_by_key_remote_info;
use function Breakdance\Licensing\get_option_receive_beta_updates;
use function Breakdance\Licensing\get_stored_license_key;
use function Breakdance\Licensing\save_option_receive_beta_updates;
use function Breakdance\Licensing\set_stored_license_key;
...
function tab()
{
    ...
    if (is_post_request() && check_admin_referer($nonce_action)) {
        if (isset($_POST['submit'])) {
            ...
            if ($key !== null && $key !== false) {
                ...
                set_stored_license_key($trimmed_key === '' ? null : $trimmed_key);
                activate_stored_license_key_at_edd();
            }
            ...
            if (false !== activate_stored_license_key_at_edd()) {
            ...
            // TODO custom notices don't work within settings tabs
            if (false !== deactivate_stored_license_key_at_edd()) {
                add_action('admin_notices', function () {
    ...
    /**
     * @var false|array{expires: string, success: boolean, license: string} $key_remote_info
     */
    $key_remote_info = $stored_key === null ? false : check_license_key_at_edd_and_upgrade_trial_if_successful($stored_key);

    $license_info = $key_remote_info === false 
        ? false 
        : get_human_readable_license_info_by_key_remote_info($key_remote_info);

    $show_activate_license_btn = $stored_key !== null
            && $key_remote_info !== false
            && isset($key_remote_info['license'])
            && $key_remote_info['license'] === 'inactive';

    $show_deactivate_license_btn = $stored_key !== null
            && $key_remote_info !== false
            && isset($key_remote_info['license'])
            && $key_remote_info['license'] === 'valid';
    ?>
    ...
    <form action="" method="post">
        ...
        <table class="form-table" role="presentation">
            ...
            <tr>
                ...
                <td>
                    <input type='text' id='key' name='key' style='width: 300px; -webkit-text-security: disc;'
                    ...
                    <p class="description">
                        If you purchased Breakdance, you can find your license key at <a href='https://breakdance.com/portal' target="_blank">https://breakdance.com/portal</a>. A license key is required to install updates in one click from the WP admin.
                    </p>
                </td>
            </tr>
            <tr>
